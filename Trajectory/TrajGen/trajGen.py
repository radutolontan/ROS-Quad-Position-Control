...
class trajGenerator:
    ...
    def optimize(self):
        diff = self.waypoints[0:-1] - self.waypoints[1:]#calculate the distance(x,y,z) between way points
        Tmin = LA.norm(diff,axis = -1)/self.max_vel#assign time through distance,now use the minimum time
        T = optimize.minimize(self.get_cost,Tmin, method="COBYLA",constraints= ({'type': 'ineq', 'fun': lambda T: T-Tmin}))['x']
        self.TS[1:] = np.cumsum(T) #time allocation
